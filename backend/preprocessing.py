...
def preprocess_data(filepath, company_data_path=None, save_dir=None):
    ...
    if company_data_path is None:
        # Always resolve from the project root, not relative to backend/
        company_data_path = str(Path(__file__).parent / 'notebooks' / 'project_info' / 'ProjectID_Detail.xlsx')
        if not os.path.exists(company_data_path):
            # Try backend/notebooks/project_info as fallback
